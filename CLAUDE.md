...
```
specs/
  entities/   ← Zod schemas (SINGLE SOURCE OF TRUTH for types)
...
- Adding or changing a field? Edit `specs/entities/*.schema.ts` FIRST.
- NEVER add `interface` or `type` manually to `shared/types/index.ts` — it re-exports from schemas.
- Use `z.infer<typeof XSchema>` everywhere, not manual type annotations.

**2. Fixtures over hardcoded mock data**
- Effects that return mock data MUST import from `specs/fixtures/`, not define data inline.
- `loadEventsFx`, `loadNotificationsFx`, etc. must return `allEvents`, `allNotifications`, etc.

**3. Scenario first, test second**
- Before implementing a new feature behavior, add it to the matching `specs/features/*.scenarios.ts`.
- Test files in `__tests__/` mirror the scenario file structure (describe/it matches scenario keys).

**4. FSD dependency direction (strictly enforced)**
```
...
- `specs/` has no imports from `app/`, `features/`, `entities/`

**5. Type drift is a bug**
- If you see the same concept defined in multiple places with different shapes, fix it by consolidating to `specs/entities/`.
- Found: `Event.time` vs `Event.startTime` — canonical is `Event.time` (HH:MM) per `specs/entities/event.schema.ts`.
- Found: `Notification.read` vs `Notification.isRead` — canonical is `read` per `specs/entities/notification.schema.ts`.

### Workflow for new features

```
1. specs/entities/ — update or add Zod schema
2. specs/fixtures/ — add deterministic fixture data
3. specs/features/ — write the behavior scenarios
4. features/*/model.ts — implement with effector (import types from @/shared/types)
5. __tests__/features/ — write tests matching scenario structure
6. app/ or widgets/ — build the UI
```
...
## Task Management — Notion Kanban

All tasks live in Notion. **Always check the board before starting work and update tickets as you go.**

- **Board**: https://www.notion.so/349ed77c0cdc80569ed0efa87f5fab7e?v=349ed77c0cdc81feb808000c422c0127
- **Data source ID**: `349ed77c-0cdc-81c1-bfa9-000b4b78b025`

### Workflow

```
1. Search Notion for tickets in the relevant Area/Phase
2. Set ticket Status → "In Progress" before starting
3. Implement (follow SDD workflow above)
4. Set ticket Status → "Done" + add Notes summary when complete
```
...
| Priority | P0 · P1 · P2 |
| Area     | routing · auth · ux · middleware · verification · refactor · calendar |
| Phase    | Audit · Spike · Foundation · Details · Rescheduling · Polish · Verification · Proxy · Auth Model · Auth Pages · Route Group |
| Sprint   | Auth Flow Sprint 1 · Backlog |

### Notion MCP tools
- `notion-search` — find tickets by keyword
- `notion-fetch` on data source ID — list all tickets and schema
- `notion-update-page` with `command: "update_properties"` — update Status/Notes
- `notion-create-pages` with `parent.data_source_id` — create new tickets
